...
app.get("/admin",(req,res)=>{
  ...
  if(req.session.userid!=null){
    ...
    db.query("SELECT * FROM user WHERE username='"+req.session.userid+"'",(err,users)=>{
...
app.all("/signin",(req,res)=>{
    if (req.method == "POST"){
        ...
        db.query(`SELECT * FROM user WHERE username = "${params.username}" AND password = "${params.password}"`,(err, result)=>{
...
app.get("/guest",(req,res)=>{
    ...
    if(req.session.userid!=null){
        ...
        db.query("SELECT * FROM user WHERE username = '"+req.session.userid+"'",(err,result)=>{
            if(err) throw err;
...
app.all("/update/:id",(req,res)=>{
    ...
    if(req.method=="POST"){
        ...
        db.query(`UPDATE items SET item_name='${params.update_item_name}', item_cat = '${params.update_item_cat}', item_price = '${params.update_item_price}',  item_quantity = '${params.update_item_quantity}',  item_picture = '${params.update_item_picture}' WHERE item_id = ${params.update_item_id} `,(err,result)=>{
            if(err) throw err;
            res.redirect("/admin");
        })
    }else{
